	...
	public function __construct( $host, $user, $pw, $name, $long = false ) {
		...
		if ( $this->conn->connect_error ) {
			switch ( $this->conn->connect_errno ) {
				...
				case 2005:
					die( "连接数据库失败，数据库地址错误或者数据库服务器不可用" );
					break;

				case 2006:
					die( "连接数据库失败，数据库服务器不可用");
					break;

				default :
					die( "连接数据库失败，请检查数据库信息。错误编号：" . $this->conn->connect_errno);
					break;
			}
		}
	...
	public function query( $sql, $noerror = false ) {
		...
		if ( ! $this->result ) {
			if ( $noerror == true ) {
				...
				die( "MySQL 语句执行错误：<br/><b>语句：</b>$sql<br/><b>错误：</b>" . $this->geterror() );
			}
		} else {
			return $this->result;
		}
	...
	public function xquery( $sql, $noerror = false ) {
		...
		if ( ! $this->result ) {
			if ( $noerror == true ) {
				...
				die( "MySQL 批量语句执行错误：<br/><b>语句：</b>$sql<br/><b>错误：</b>" . $this->geterror());
			}
		} else {
			return $this->result;
		}
